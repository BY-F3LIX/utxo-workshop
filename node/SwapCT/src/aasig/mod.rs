...
impl<Algo: TaggedRingSig + Default + Send + Sync + Clone, Msg: AAMsg + Send + Sync> AASig<Algo, Msg> {
    pub fn sign(signers: &Vec<(&Vec<OTAccount>, &usize, &TypeCommitment)>, msgs: &Vec<Msg>) -> Result<AASig<Algo, Msg>, AAError> {
        ...
        let outside: Vec<(Scalar, Scalar, (VSigmaProof, CompressedRistretto, Msg))> = msgs.iter().zip(sr_exp).map(|(msg,(s_exp,r_exp))|{
        ...
        let sigs: Vec<Result<(VSigmaProof, CompressedRistretto, Algo, CompressedRistretto ),AAError>> = signers.iter().zip(random_exponents).map(|((accts,index, com),random_exponent)| {
    ...
    pub fn verify(&self, signers: &Vec<(&Vec<OTAccount>, &TypeCommitment)>, msgs: &Vec<Msg>) -> Result<(), AAError> {
        ...
        let vers: Vec<Result<(usize,Scalar, RistrettoPoint),AAError>> = self.output_proofs.iter().map(|(proof, com, msg)| {
        ...
        let verout: Vec<Result<(usize,RistrettoPoint),AAError>> = self.input_proofs.iter().map( |(proof, com, signature, intcom)| {
            let mut transcript = Transcript::new(b"input knowledge");
